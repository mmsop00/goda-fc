// GET /api/donations/receipts/[id]/image → ảnh bill ủng hộ (chỉ chủ tịch xem để đối chiếu)
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!receipt) return NextResponse.json({ error: "Không tìm thấy ảnh" }, { status: 404 });
  if (receipt.expiresAt < new Date()) {
    await prisma.donationReceipt.delete({ where: { id } });
    return NextResponse.json({ error: "Ảnh đã hết hạn lưu trữ" }, { status: 404 });
  }
  return new NextResponse(new Uint8Array(receipt.imageData), {
    headers: { "Content-Type": receipt.mimeType, "Cache-Control": "private, max-age=3600" },
  });
}
