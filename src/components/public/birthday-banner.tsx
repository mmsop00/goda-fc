...
export function BirthdayBanner({ members, events, recentDonations = [] }: BirthdayBannerProps) {
  ...
  // lặp lại bộ chip đủ số lần để phủ kín. Tốc độ cố định ~60px/giây trên mọi màn hình.
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setRepeat(n);
      setDuration(Math.max(6, Math.round((setWidth * n) / 60)));
    };
    measure();
  ...
  return (
    <div ref={containerRef} className="gd-ticker relative bg-goda-navy border-y border-goda-yellow/40 overflow-hidden">
      ...
      <div className="flex py-2.5">
        <div className="gd-ticker-track flex items-center shrink-0" style={{ animationDuration: `${duration}s`, ["--gd-dur" as string]: `${duration}s` }}>
          {renderTrack("a")}
        </div>
        <div aria-hidden className="gd-ticker-track flex items-center shrink-0" style={{ animationDuration: `${duration}s`, ["--gd-dur" as string]: `${duration}s` }}>
          {renderTrack("b")}
        </div>
      </div>
      ...
      <style>{`
        ...
        /* Chỉ dừng khi rê chuột thật; trên điện thoại chạm vào sẽ "dính" hover
           làm bảng tin đứng yên luôn. Máy bật "giảm chuyển động" vẫn chạy (chậm
           hơn) thay vì tắt hẳn, để không ai thấy bảng tin bị đứng. */
        @media (hover: hover) and (pointer: fine) {
          .gd-ticker:hover .gd-ticker-track { animation-play-state: paused; }
        }
        @media (prefers-reduced-motion: reduce) {
          .gd-ticker-track { animation-duration: calc(var(--gd-dur) * 1.6) !important; }
        }
      `}</style>
    </div>
  );
