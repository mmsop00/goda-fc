...
/** Trang Thành viên — `members` đã ghép hồ sơ thành viên tự sửa (server truyền vào). */
export function ThanhVienView({ members }: { members: MemberPublic[] }) {
  const [position, setPosition] = useState<MemberPosition | "all">("all");

  const filtered =
    position === "all"
      ? members
      : members.filter((m) => m.position === position);

  // Top stats
  const topGoals = useMemo(() => [...members].sort((a, b) => b.goals - a.goals).slice(0, 5), [members]);
  const topAssists = useMemo(() => [...members].sort((a, b) => b.assists - a.assists).slice(0, 5), [members]);
  const topMVP = useMemo(() => [...members].sort((a, b) => b.mvp - a.mvp).slice(0, 5), [members]);
